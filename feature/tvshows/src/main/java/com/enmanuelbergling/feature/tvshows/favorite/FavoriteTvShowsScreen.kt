...
@Composable
fun FavoriteTvShowsRoute(
    onTvShowsDetails: (tvShowId: Int) -> Unit,
    onBack: () -> Unit,
) {
    val viewModel = koinViewModel<FavoriteTvShowsVM>()
    ...
    val removeTvShowsErrorMessage =
        stringResource(R.string.the_tv_show_could_not_be_removed_from_favorites)
    val tvShowRemovedMessage = stringResource(R.string.tv_show_removed_from_favorites)
    ...
    ObserveAsEvents(viewModel.sideEffectChannel) {
        when (it) {
            is FavoriteTvShowsSideEffect.NavigateToDetails -> onTvShowsDetails(it.tvShowId)
            is FavoriteTvShowsSideEffect.UndoRemoveTvShows -> scope.launch {
                ...
                when (result) {
                    SnackbarResult.Dismissed -> viewModel.onEvent(FavoriteTvShowsEvent.RemoveTvShows(it.tvShowId))
                    SnackbarResult.ActionPerformed -> viewModel.onEvent(FavoriteTvShowsEvent.UndoRemove)
                }
            }

            is FavoriteTvShowsSideEffect.RemoveTvShowsError -> scope.launch {
                val result = snackbarHostState.showSnackbar(
                ...
                when (result) {
                    SnackbarResult.Dismissed -> viewModel.onEvent(
                        FavoriteTvShowsEvent.OnRemoveTvShowsErrorDismissed(it.tvShowId)
                    )

                    SnackbarResult.ActionPerformed -> viewModel.onEvent(
                        FavoriteTvShowsEvent.RemoveTvShows(it.tvShowId)
                    )
                }
            }
        }
...
@Composable
private fun FavoriteTvShowsScreen(
    favorites: LazyPagingItems<TvShow>,
    uiState: FavoriteTvShowsState,
    snackbarHostState: SnackbarHostState,
    onBack: () -> Unit,
    onEvent: (FavoriteTvShowsEvent) -> Unit,
) {
    ...
    Scaffold(
        ...
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text(text = stringResource(R.string.favorite_tv_shows)) },
                navigationIcon = {
                    ...
                    items(favorites, key = { tvShow -> tvShow.id }) { tvShow ->
                        tvShow?.let {
                            TinderSwipeToDismissContainer(
                                visible = tvShow.id !in uiState.deletedTvShowsIds,
                                onDismissFromStartToEnd = {
                                    onEvent(FavoriteTvShowsEvent.NavigateToDetails(tvShow.id))
                                },
                                onDismissFromEndToStart = {
                                    onEvent(FavoriteTvShowsEvent.OnRemoveTvShows(tvShow.id))
                                },
