...
import com.enmanuelbergling.feature.tvshows.search.ExpandedSearchBarContent
...
fun TvShowsScreen(
    ...
    onMore: (TvShowsSection) -> Unit,
    onFavorites: () -> Unit,
    onOpenDrawer: () -> Unit,
) {
    val viewModel = koinViewModel<TvShowsVM> { parametersOf(false) }
    ...
    val tvShowSearch = viewModel.tvShowSearch.collectAsLazyPagingItems()
...
private fun ExpandedSearchBarInputField(
    ...
    SearchBarDefaults.InputField(
        ...
        placeholder = {
            Text(text = stringResource(R.string.tv_shows))
        },
    )
}
...
private fun TvShowsList(
    ...
    if (isLoading) Box(contentAlignment = Alignment.Center, modifier = Modifier.fillMaxSize()) {
        ...
        if (popular.isNotEmpty()) headerTvShows(
            popular = popular,
            onDetails = onDetails,
        ) { onMore(TvShowsSection.Popular) }

        if ((topRated + onTheAir + airingToday).isNotEmpty()) forYouText()

        if (topRated.isNotEmpty()) tvShowRowSection(
            title = R.string.top_rated,
            tvShows = topRated,
            onDetails = onDetails,
        ) { onMore(TvShowsSection.TopRated) }

        if (onTheAir.isNotEmpty()) tvShowRowSection(
            title = R.string.on_the_air,
            tvShows = onTheAir,
            onDetails = onDetails,
        ) { onMore(TvShowsSection.OnTheAir) }

        if (airingToday.isNotEmpty()) tvShowColumnSection(
            title = R.string.airing_today,
            tvShows = airingToday,
            onDetails = onDetails,
        ) { onMore(TvShowsSection.AiringToday) }
    }
}

private fun LazyListScope.tvShowRowSection(
    @StringRes title: Int,
    tvShows: List<TvShow>,
    onDetails: (id: Int) -> Unit,
    onMore: () -> Unit,
...
@OptIn(ExperimentalMaterial3ExpressiveApi::class)
private fun LazyListScope.tvShowColumnSection(
    @StringRes title: Int,
    tvShows: List<TvShow>,
    onDetails: (id: Int) -> Unit,
    onMore: () -> Unit,
...
private fun LazyListScope.headerTvShows(
    popular: List<TvShow>,
    onDetails: (id: Int) -> Unit,
    onMore: () -> Unit,
) {
